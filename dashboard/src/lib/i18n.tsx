...
export const UI_TRANSLATIONS: Record<string, Translation> = {
  ...
  'Pendiente de condición': { en: 'Waiting for required condition', it: 'In attesa della condizione richiesta', de: 'Warten auf erforderliche Bedingung' },
  'No aplicable': { en: 'Not applicable', it: 'Non applicabile', de: 'Nicht anwendbar' },
  'Hardware no compatible': { en: 'Hardware not compatible', it: 'Hardware non compatibile', de: 'Hardware nicht kompatibel' },
  'Nunca se inventa un valor.': { en: 'A value is never invented.', it: 'Non viene mai inventato un valore.', de: 'Ein Wert wird niemals erfunden.' },
  '“Pendiente” significa que el agente la identificó como posible, pero falta comprobarla o terminar su decodificación. “No disponible” significa que la ECU la rechazó, no equipa el sensor o no es accesible con el hardware actual. Nunca se inventa un valor.': { en: '“Pending” means the agent identified it as possible, but it still needs verification or decoding. “Unavailable” means the ECU rejected it, the sensor is not fitted, or the current hardware cannot access it. A value is never invented.', it: '“In attesa” significa che l’agente l’ha identificata come possibile, ma deve ancora verificarla o terminarne la decodifica. “Non disponibile” significa che la ECU l’ha rifiutata, il sensore non è installato o l’hardware attuale non può accedervi. Non viene mai inventato un valore.', de: '„Ausstehend“ bedeutet, dass der Agent die Messgröße als möglich erkannt hat, sie aber noch geprüft oder fertig dekodiert werden muss. „Nicht verfügbar“ bedeutet, dass die ECU sie abgelehnt hat, der Sensor nicht verbaut ist oder die aktuelle Hardware keinen Zugriff ermöglicht. Ein Wert wird niemals erfunden.' },
...
const dynamicTranslate = (value: string, language: AppLanguage): string | null => {
  ...
  const numberPatterns: Array<[RegExp, Record<'en' | 'it' | 'de', string>]> = [
    ...
    [/^(\d+) métricas confirmadas$/, { en: '$1 confirmed metrics', it: '$1 metriche confermate', de: '$1 bestätigte Messgrößen' }],
    [/^(\d+) pendientes de comprobar o decodificar$/, { en: '$1 pending verification or decoding', it: '$1 in attesa di verifica o decodifica', de: '$1 ausstehende Prüfungen oder Dekodierungen' }],
    [/^(\d+) métricas catalogadas$/, { en: '$1 catalogued metrics', it: '$1 metriche catalogate', de: '$1 katalogisierte Messgrößen' }],
